...
import { LinkedListCanvas } from '@/components/linked-lists/LinkedListCanvas'
import { Button } from '@/components/ui/button'
import {
  LINKED_LIST_ALGORITHMS,
  type LinkedListAlgorithmId
...
interface LinkedListsViewProps {
  highlightedCodeByAlgo: Record<LinkedListAlgorithmId, HighlightedCodeSamples>
  initialAlgo: LinkedListAlgorithmId
  initialValues: number[]
}

export function LinkedListsView({
  highlightedCodeByAlgo,
  initialAlgo,
  initialValues
}: LinkedListsViewProps) {
  const [algorithm, setAlgorithm] = useState<LinkedListAlgorithmId>(initialAlgo)
  ...
  const frames = useMemo(
    () => Array.from(LINKED_LIST_ALGORITHMS[algorithm].run(values)),
    [algorithm, values]
  )
  const totalSteps = Math.max(frames.length - 1, 0)
  const frame = frames[Math.min(step, totalSteps)] ?? frames[0]
  const activeAlgorithm = LINKED_LIST_ALGORITHMS[algorithm]
  ...
  function applyValues() {
    const parsed = parseArrayInput(valueInput).slice(0, 8)
    setValues(parsed)
    ...
    <main className="responsive-page flex flex-col gap-6 lg:gap-8">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-4 max-w-3xl text-base leading-7 text-foreground/75">
            Trace prev, current, and next pointers as each node rewires its
            next reference to reverse the list in place.
          </p>
        </div>
        ...
        <div className="glass-panel flex flex-wrap items-center gap-3 rounded-lg p-3">
          ...
          <select
            ...
            onChange={(event) =>
              setAlgorithm(event.target.value as LinkedListAlgorithmId)
            }
            value={algorithm}
          >
            {Object.values(LINKED_LIST_ALGORITHMS).map((item) => (
      ...
      <section className="visualizer-grid">
        ...
        <CodePanel
          ...
          docsHref={`/docs#linked-lists-${algorithm}`}
