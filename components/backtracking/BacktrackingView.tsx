...
import { BacktrackingCanvas } from '@/components/backtracking/BacktrackingCanvas'
import { CodePanel } from '@/components/code/CodePanel'
import { Button } from '@/components/ui/button'
import {
  BACKTRACKING_ALGORITHMS,
  type BacktrackingAlgorithmId
...
interface BacktrackingViewProps {
  highlightedCodeByAlgo: Record<BacktrackingAlgorithmId, HighlightedCodeSamples>
  initialAlgo: BacktrackingAlgorithmId
}

export function BacktrackingView({
  highlightedCodeByAlgo,
  initialAlgo
}: BacktrackingViewProps) {
  const [algorithm, setAlgorithm] = useState<BacktrackingAlgorithmId>(initialAlgo)
  const [step, setStep] = useState(0)
  const [isPlaying, setIsPlaying] = useState(false)
  const [speed, setSpeed] = useState(1)

  const frames = useMemo(
    () => Array.from(BACKTRACKING_ALGORITHMS[algorithm].run()),
    [algorithm]
  )
  const totalSteps = Math.max(frames.length - 1, 0)
  const frame = frames[Math.min(step, totalSteps)] ?? frames[0]
  const activeAlgorithm = BACKTRACKING_ALGORITHMS[algorithm]
    ...
    <main className="responsive-page flex flex-col gap-6 lg:gap-8">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-4 max-w-3xl text-base leading-7 text-foreground/75">
            Step through choose, explore, reject, and undo moments while the
            path, board state, result count, time, and space complexity update.
          </p>
        </div>
        ...
        <div className="glass-panel flex flex-wrap items-center gap-3 rounded-lg p-3">
          ...
          <select
            ...
            onChange={(event) =>
              setAlgorithm(event.target.value as BacktrackingAlgorithmId)
            }
            value={algorithm}
          >
            {Object.values(BACKTRACKING_ALGORITHMS).map((item) => (
      ...
      <section className="visualizer-grid">
        ...
        <div className="flex flex-col gap-4">
          <BacktrackingCanvas frame={frame} />
          <div className="grid gap-4 md:grid-cols-3">
            <Metric label="Results" value={String(frame.results.length)} />
        ...
        <CodePanel
          ...
          docsHref={`/docs#backtracking-${algorithm}`}
