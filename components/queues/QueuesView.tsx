...
import { QueueCanvas } from '@/components/queues/QueueCanvas'
import { Button } from '@/components/ui/button'
import {
  QUEUE_ALGORITHMS,
  type QueueAlgorithmId
} from '@/engine/queues'
...
interface QueuesViewProps {
  highlightedCodeByAlgo: Record<QueueAlgorithmId, HighlightedCodeSamples>
  initialAlgo: QueueAlgorithmId
...
export function QueuesView({
  ...
  const [algorithm, setAlgorithm] = useState<QueueAlgorithmId>(initialAlgo)
  ...
  const frames = useMemo(
    () => Array.from(QUEUE_ALGORITHMS[algorithm].run(values)),
    [algorithm, values]
  )
  const totalSteps = Math.max(frames.length - 1, 0)
  const frame = frames[Math.min(step, totalSteps)] ?? frames[0]
  const activeAlgorithm = QUEUE_ALGORITHMS[algorithm]
  ...
  function applyValues() {
    const parsed = parseArrayInput(valueInput).slice(0, 12)
    setValues(parsed)
    ...
    <main className="responsive-page flex flex-col gap-6 lg:gap-8">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-4 max-w-3xl text-base leading-7 text-foreground/75">
            Visualize FIFO behavior with enqueue, dequeue, and peek operations
            before moving into sliding window and scheduling patterns.
          </p>
        </div>
        ...
        <div className="glass-panel flex flex-wrap items-center gap-3 rounded-lg p-3">
          ...
          <select
            ...
            onChange={(event) => setAlgorithm(event.target.value as QueueAlgorithmId)}
            value={algorithm}
          >
            {Object.values(QUEUE_ALGORITHMS).map((item) => (
      ...
      <section className="visualizer-grid">
        ...
        <CodePanel
          ...
          docsHref={`/docs#queues-${algorithm}`}
