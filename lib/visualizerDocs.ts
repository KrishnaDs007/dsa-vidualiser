...
export const VISUALIZER_DOCS: VisualizerDoc[] = [
  ...
  doc('backtracking-subsets', 'Backtracking', 'Subsets', '/backtracking?algo=subsets', 'Builds every include/exclude decision path and records each subset at the leaf.', 'O(2^n) time / O(n) space', ['Decision tree'], ['Include the current value.', 'Explore deeper.', 'Backtrack and exclude it.']),
  doc('backtracking-permutations', 'Backtracking', 'Permutations', '/backtracking?algo=permutations', 'Chooses unused values into a path until one full ordering is formed.', 'O(n!) time / O(n) space', ['Used set'], ['Skip used values.', 'Choose one unused value.', 'Undo the choice after exploring.']),
  doc('backtracking-combinations', 'Backtracking', 'Combinations', '/backtracking?algo=combinations', 'Uses a start index so combinations grow without repeating earlier choices.', 'O(C(n,k)) time / O(k) space', ['Start index'], ['Choose a value.', 'Recurse from the next index.', 'Backtrack to try the next choice.']),
  doc('backtracking-nQueens', 'Backtracking', 'N-Queens', '/backtracking?algo=nQueens', 'Places one queen per row and rejects columns or diagonals already attacked.', 'O(n!) time / O(n) space', ['Constraint pruning'], ['Try a square.', 'Reject attacked positions.', 'Place and later remove the queen.']),
  doc('backtracking-sudoku', 'Backtracking', 'Sudoku Solver', '/backtracking?algo=sudoku', 'Fills empty cells by trying legal digits and backtracking when a later cell cannot be solved.', 'O(4^empty) time / O(empty) space', ['Constraint search'], ['Find an empty cell.', 'Try safe digits.', 'Undo a digit when the branch fails.']),
  doc('greedy-activitySelection', 'Greedy', 'Activity Selection', '/greedy?algo=activitySelection', 'Selects the activity that finishes earliest whenever it does not overlap the last selected activity.', 'O(n log n) time / O(1) space', ['Interval sorting'], ['Sort by finish time.', 'Accept compatible intervals.', 'Reject overlaps.']),
  doc('greedy-mergeIntervals', 'Greedy', 'Merge Intervals', '/greedy?algo=mergeIntervals', 'Sorts intervals by start and merges each overlap into the current interval.', 'O(n log n) time / O(n) space', ['Interval scan'], ['Sort by start.', 'Start a new interval when separated.', 'Extend the last interval when overlapping.']),
  doc('greedy-jumpGame', 'Greedy', 'Jump Game', '/greedy?algo=jumpGame', 'Keeps the farthest reachable index and succeeds once that reach covers the end.', 'O(n) time / O(1) space', ['Reachability'], ['Scan reachable indexes.', 'Update farthest reach.', 'Stop when the end is covered.']),
  doc('greedy-gasStation', 'Greedy', 'Gas Station', '/greedy?algo=gasStation', 'Resets the candidate start whenever the running tank becomes negative.', 'O(n) time / O(1) space', ['Running balance'], ['Track gas minus cost.', 'Reset after a negative tank.', 'Return the surviving start if total balance works.']),
  doc('greedy-huffman', 'Greedy', 'Huffman Coding', '/greedy?algo=huffman', 'Repeatedly merges the two lowest-frequency nodes to build an optimal prefix tree.', 'O(n log n) time / O(n) space', ['Priority queue'], ['Pop two smallest nodes.', 'Merge their weights.', 'Push the merged node back.'])
]
